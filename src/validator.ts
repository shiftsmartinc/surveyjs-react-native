...
const getErrorStr = (
  ...
  const errorMsg = errorText
    ? formatStringWithResults(errorText, results)
    : stringFormat(errorTemplates[templateName], ...errorArgs);
  return errorMsg;
};
...
export default class QuestionValidator {
  ...
  getValidators() {
    ...
    if (
      this.owner.value &&
      (this.owner.json.maxLength || this.owner.json.minLength)
    ) {
  ...
  runCondition(consumer, left, right) {
    ...
    if (rightValue.variableName) {
      ...
      rightRawValue = getResponseValue(
        this.owner.collection.results,
    ...
    if (leftValue.value) {
      ...
      if (!leftQuestion) return null;
      if (leftValue.variableName) {
        leftRawValue = getResponseValue(
          this.owner.collection.results,
    ...
    const operationResult = consumer(
      getValue(leftRawValue),
      getValue(rightRawValue),
    );
